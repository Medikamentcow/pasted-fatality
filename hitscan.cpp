...
void hitscan::multipoint_hitbox(circular_buffer<aimbot::aimpoint_t, 78>& points, int index, const studiohdr_t* studio_hdr, lag_record_t& record, const Vector& eyepos, float spread_radius, bool is_knife, bool is_zeus, const weaponconfig_t* config, int safety, lag_record_t* extra, CCSWeaponData* weapon_data)
{
	...
	for (auto i = 0; i < 3; i++)
	{
		...
		for (auto j = resolver_direction::resolver_networked; j < resolver_direction::resolver_direction_max; j++)
		{
			...
			{
				auto& p = proj[k];
				auto v_off = p - orig_center;
				auto dist = v_off.x * fw.x + v_off.y * fw.y + v_off.z * fw.z;
				p = p - fw * dist;
			}
	...
	const auto extra_head_calc = [&](const Vector& top, const Vector& bot)
		{
			...
			for (auto i = 0u; i < 6; i++)
			{
				...
				if (!(penetration::trace_to_studio_csgo_hitgroups_priority(&ray, player->get_model_ptr(), hitbox_set, hitboxbones, MASK_SHOT_HULL | CONTENTS_HITBOX, &record.m_origin, player->get_model_scale(), &enter_trace) && enter_trace.hitgroup == hitbox->group))
					break;
			}
	...
	full = poly_intersect::get_intersection_poly(full, mapped_points[0][static_cast<int>(resolver_direction::resolver_min_extra)]);
	full = poly_intersect::get_intersection_poly(full, mapped_points[0][static_cast<int>(resolver_direction::resolver_max_extra)]);

	if (record.m_last_ang_differs)
	{
		full = poly_intersect::get_intersection_poly(full, mapped_points[1][static_cast<int>(resolver_direction::resolver_min_min)]);
		full = poly_intersect::get_intersection_poly(full, mapped_points[1][static_cast<int>(resolver_direction::resolver_max_max)]);
		full = poly_intersect::get_intersection_poly(full, mapped_points[1][static_cast<int>(resolver_direction::resolver_min_extra)]);
		full = poly_intersect::get_intersection_poly(full, mapped_points[1][static_cast<int>(resolver_direction::resolver_max_extra)]);
	}

	if (use_extra)
	{
		full = poly_intersect::get_intersection_poly(full, mapped_points[2][static_cast<int>(resolver_direction::resolver_min_min)]);
