...
bool lagcomp::run_extrapolation(const C_CSPlayer* player, const bool simple)
{
	...
	const auto& newest = log.record.back();

	// ------------------------------------------------------------------
	// Case 1: We already have the highest simtime (or the record is ignored)
	// Just reuse that record – no real extrapolation needed
	// ------------------------------------------------------------------
	if (ConVar::cl_lagcompensation && ConVar::cl_predict &&
		(newest.m_simtime < log.m_highest_simtime ||
			(newest.m_simtime == log.m_highest_simtime && newest.m_ignore)))
	{
	...
	// ------------------------------------------------------------------
	// Case 2: Normal extrapolation
	// ------------------------------------------------------------------
	const auto* record = &newest;
	const float rtt = misc::get_latency();
	const int lag = std::max(1, record->m_lagamt);
	const int ticks_behind = record->ticks_behind(lag);

	log.extrapolated_record = *record;
	log.extrapolated_record.m_breaking_lc = record->breaking_lc();
	log.extrapolated_record.m_shot_info.breaking_lc = log.extrapolated_record.m_breaking_lc;

	const int possible_future_tick = interfaces::client_state()->get_last_server_tick() + 1 + time_to_ticks(rtt) + 8;
	const int max_future_ticks = possible_future_tick - time_to_ticks(log.extrapolated_record.m_simtime + get_lerp_time());

	bool to_server_position = log.extrapolated_record.m_breaking_lc;

	// Try to find the furthest valid extrapolation distance
	if (!to_server_position)
	{
		...
		{
			const int candidate = clamp((i + ticks_behind) / lag * lag, 0, 64);
			if (valid_simtime(record->m_simtime + ticks_to_time(candidate)))
				best_lagamt = candidate;
			else
				break;	// once we hit an invalid one, further ones will also be invalid
		}

		if (best_lagamt > 0)
			log.extrapolated_record.m_lagamt = best_lagamt;
		else
			to_server_position = true;
	}

	// Prefer the "server position" amount when it is safer / smaller
	const int server_position = clamp((time_to_ticks(rtt) + ticks_behind) / lag * lag, 0, 64);
	if (to_server_position || server_position < log.extrapolated_record.m_lagamt)
		log.extrapolated_record.m_lagamt = server_position;

	// Final safety clamp
	log.extrapolated_record.m_lagamt = clamp(log.extrapolated_record.m_lagamt, 0, 64);

	log.extrapolated_record.m_tickbase_shift = false;

	// Perform the actual extrapolation
	...
	log.extrapolated_record.m_valid = true;
	log.extrapolated_record.m_lagamt = lag;		// restore original lag amount for the record
	log.extrapolated_record.m_shot_info.has_info = true;
	log.extrapolated_record.m_shot_info.delay_shot = false;
