...
void antiaim::run_on_send()
{
	...
	if (in_flip && vars::aa.jitter->get<bool>() && vars::aa.flip_fake->get<bool>() && !is_defusing && on_non_lc_tick())
		target_direction = -target_direction;
...
bool antiaim::on_non_lc_tick()
{
	// Choked tick
	if (!*globals::send_packet)
		return true;

	// Defensive / tickbase shift
	if (tickbase::force_choke || tickbase::to_shift > 0 || tickbase::to_recharge > 0)
		return true;

	// Simtime itself would fail lagcomp
	const float simtime = ticks_to_time(local_player->get_tickbase());
	if (!lagcomp::valid_simtime(simtime))
		return true;
...
void antiaim::do_jitter(float& yaw)
{
	...
	// Prefer full jitter on non-LC ticks, reduced on LC-able ticks
	if (!on_non_lc_tick())
		jitter_range *= 0.7f;	// or 0.f to fully suppress on LC ticks
